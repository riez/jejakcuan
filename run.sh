...
run_api_watch() {
  echo -e "\n${BLUE}Starting Rust API server with hot reload...${NC}"
  cd "$PROJECT_ROOT"
  ...
  cargo watch -x 'run --bin jejakcuan-api' -w apps/api -w crates
}
...
run_dev() {
  ...
  echo -e "\n${GREEN}Starting all services in DEVELOPMENT mode with hot reload...${NC}"
  echo -e "${YELLOW}Press Ctrl+C to stop all services${NC}\n"

  if ! command -v cargo-watch >/dev/null 2>&1; then
    print_warning "Installing cargo-watch for hot reload..."
    cargo install cargo-watch
  fi

  (cd "$PROJECT_ROOT" && cargo watch -x 'run --bin jejakcuan-api' -w apps/api -w crates 2>&1 | sed 's/^/[API] /') &
  API_PID=$!
  ...
  echo -e "\n${GREEN}Development services running with HOT RELOAD:${NC}"
  echo -e "  Frontend: ${BLUE}http://localhost:5173${NC} (Vite HMR)"
  echo -e "  API:      ${BLUE}http://localhost:8080${NC} (cargo-watch)"
  echo -e "  Postgres: ${BLUE}localhost:5432${NC}"
  echo -e "  Redis:    ${BLUE}localhost:6379${NC}"
  echo ""
  echo -e "${YELLOW}Changes to Rust files will auto-rebuild. Changes to Svelte files are instant.${NC}"
  echo ""
...
show_help() {
  ...
  echo ""
  echo "Examples:"
  echo "  ./run.sh dev      # Start with hot reload (recommended for development)"
  echo "  ./run.sh start    # Start everything (production-like)"
